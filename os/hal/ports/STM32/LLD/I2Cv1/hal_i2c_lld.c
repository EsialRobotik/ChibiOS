...
msg_t i2c_lld_matchAddress(I2CDriver *i2cp, i2caddr_t  addr)
{
  ...
  if (ownAdr == 0 || ownAdr == i2cadr)
    dp->OAR1 = i2cadr | 0x4000; //14 bit should always be kept at 1 by software
  else
    return MSG_RESET;    /* cannot add this address to set of those matched */
 ...
 * @notapi
 */
msg_t i2c_lld_slave_start_transmission(I2CDriver *i2cp, const uint8_t *txbuf, size_t txbytes)
{
  I2C_TypeDef *dp = i2cp->i2c;
  systime_t start, end;
  /* TX DMA setup.*/
